...
export const API_QUERY_PARAMS = {
  ...
  filtering: {
    ...
    exists: (property: string, exists: boolean) => `exists[${property}]=${exists}`,
  ...
  sorting: {
    ...
    order: (property: string, direction: 'asc' | 'desc') => `order[${property}]=${direction}`
  }
} as const;
...
export const API_ERROR_HANDLERS = {
  ...
  handleValidationError: (error: any): string[] => {
    if (error.violations) {
      return error.violations.map((v: any) => `${v.propertyPath}: ${v.message}`);
    }
