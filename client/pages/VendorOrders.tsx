    ...
    <div className="min-h-screen bg-gradient-to-br from-app-pink via-app-purple to-app-blue relative overflow-hidden">
      ...
      <div className="relative z-10 pt-12 pb-8">
        <div className="flex items-center justify-between px-6">
          <Link to="/vendor-dashboard" className="text-white hover:scale-110 transition-transform duration-300">
            <ArrowLeft className="w-6 h-6" />
          </Link>
          <div className="text-center">
            <h1 className="text-2xl font-bold text-white drop-shadow-lg">Mes Commandes</h1>
            <p className="text-white/80 text-sm mt-1">{filteredOrders.length} commande{filteredOrders.length > 1 ? 's' : ''}</p>
          </div>
      ...
      <div className="relative z-10 px-6 mb-6">
        <div className="bg-white/20 backdrop-blur-xl rounded-2xl p-2">
          <div className="grid grid-cols-2 gap-2 mb-2">
            ...
            >
              Nouvelles ({orders.filter(o => o.status === "paid_by_donator").length})
            </button>
          </div>
          <div className="grid grid-cols-2 gap-2">
            ...
            >
              Prêtes ({orders.filter(o => o.status === "ready_for_pickup").length})
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="relative z-10 px-6 pb-24">
        {filteredOrders.length === 0 ? (
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl p-8 text-center shadow-xl border border-white/20">
            ...
            <p className="text-gray-500">Pas de commandes dans cette catégorie</p>
          </div>
        ) : (
          <div className="space-y-4">
            {filteredOrders.map((order) => {
              ...
              return (
                <div key={order.id} className="bg-white/95 backdrop-blur-xl rounded-3xl p-6 shadow-xl border border-white/20">
                  {/* Order Header */}
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <h3 className="font-bold text-gray-800 text-lg">Commande #{order.id}</h3>
                  ...
                  {/* Status Badge */}
                  <div className={`inline-flex items-center gap-2 px-3 py-2 rounded-xl ${status.bgColor} ${status.borderColor} border mb-4`}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="bg-gray-50 rounded-xl p-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <a href={`tel:${order.customerPhone}`} className="text-xs text-app-purple hover:underline">
                            {order.customerPhone}
                          </a>
                        </div>
                      </div>
                    ...
                    <div className="bg-gray-50 rounded-xl p-3">
                      ...
                      <div className="flex items-center gap-2">
                        <img src={order.donatorAvatar} alt={order.donatorName} className="w-8 h-8 rounded-full" />
                        <div>
                          <p className="font-semibold text-gray-800 text-sm">{order.donatorName}</p>
                  ...
                  <div className="bg-gray-50 rounded-xl p-3 mb-4">
                    <p className="text-xs text-gray-500 mb-2">{order.items.length} article{order.items.length > 1 ? 's' : ''}</p>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      {order.pickupCode && (
                        <p className="text-sm text-gray-500">Code: {order.pickupCode}</p>
                      )}
                    </div>
                    <div className="flex gap-2">
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-gray-800">Commande #{selectedOrder.id}</h3>
              <button
            ...
            {/* Customer Contact */}
            <div className="mb-6">
              <h4 className="font-semibold text-gray-800 mb-3">Contact client</h4>
            ...
            <div className="border-t pt-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-2xl font-bold text-app-purple">{selectedOrder.total.toFixed(2)}€</span>
              </div>
            </div>
          </div>
        </div>
